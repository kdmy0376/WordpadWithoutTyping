...
public class TextReco extends Activity {
	...
	private static String intentWord;
			...
			public void run() {
				...
				if (words.size() > 0) {
					...
					for (String word : words) {
						...
						Button btn = new Button(TextReco.this);
						btn.setText(word);
						...
						btn.setLayoutParams(txtParams);
						btn.setGravity(Gravity.CENTER_VERTICAL
								| Gravity.CENTER_HORIZONTAL);
						btn.setTextSize(textInfo[0]-6);
						btn.setHeight(textInfo[1]);
						btn.setOnTouchListener(new OnTouchListener() {
							@Override
							public boolean onTouch(View arg0, MotionEvent arg1) {
								// TODO Auto-generated method stub
								Intent intent = new Intent(getApplicationContext(), WordDetailView.class);
								intent.putExtra("word", ((Button)arg0).getText().toString());
								intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
		            			getApplication().startActivity(intent);
								return true;
							}
						});
						wordListLayout.addView(btn);
					}
				}
			}
		});
	...
	private void showLoupe(boolean isActive) {
		...
		if (isActive) {
			...
			params = (LinearLayout.LayoutParams) wordListLayout
					.getLayoutParams();
